                                  ...
                                  double* b, int kmax, double rtol)
{
  ...
  CHECK_CUDA(cudaMemcpy(d_p, b, M * sizeof(double), cudaMemcpyHostToDevice));
  CHECK_CUDA(
      cudaMemcpy(d_r, d_p, M * sizeof(double), cudaMemcpyDeviceToDevice));
  ...
  CHECK_CUDA(cudaMemcpy(d_scalar_one, &scalar_one, sizeof(double),
  ...
  while (k < kmax) {
    ...
    CHECK_CUDA(cudaMemcpy(d_rnorm_old, d_rnorm_new, sizeof(double),
