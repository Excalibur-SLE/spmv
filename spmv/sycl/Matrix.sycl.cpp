...
//-----------------------------------------------------------------------------
template <typename T>
void Matrix<T>::mult(sycl::queue& queue, sycl::buffer<T>& x_buf,
                     sycl::buffer<T>& y_buf) const
{
  if (_symmetric)
    spmv_sym_sycl(queue, x_buf, y_buf);
  else
    spmv_sycl(queue, x_buf, y_buf);
}
//-----------------------------------------------------------------------------
template <typename T>
void Matrix<T>::spmv_sycl(sycl::queue& queue, sycl::buffer<T>& x_buf,
                          sycl::buffer<T>& y_buf) const
{
  namespace acc = sycl::access;
  auto device = queue.get_device();
        ...
        h.parallel_for(
            ...
            [=](sycl::nd_item<1> item) {
              ...
              if (global_id < num_work_items) {
                ...
                T sum = 0;
                // FIXME This can also be local if all items are in single line
        ...
        h.parallel_for(sycl::range<1>{static_cast<size_t>(num_work_groups)},
                       [=](sycl::id<1> it) {
                         ...
                         if (global_carry_row[wg] < nrows) {
                           // needs atomic
                           y[global_carry_row[wg]] += global_carry_val[wg];
                         }
                       });
      });
    }
...
//-----------------------------------------------------------------------------
template <typename T>
void Matrix<T>::spmv_sym_sycl(sycl::queue& queue, sycl::buffer<T>& x_buf,
                              sycl::buffer<T>& y_buf) const
{
  namespace acc = sycl::access;
  auto device = queue.get_device();
