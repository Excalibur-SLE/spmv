...
static void test_spmv(bool symmetric)
{
  int mpi_rank;
  ...
  double norm_ref = y_ref.squaredNorm();
  ...
  l2g->update(x_local);
  if (A->symmetric())
    A->spmv_sym_sycl(queue, x_local, y_local).wait();
  else
    A->spmv_sycl(queue, x_local, y_local).wait();
#else
...
int main(int argc, char** argv)
{
  ...
  bool symmetric;
  if (mpi_rank == 0)
    std::cout << "Running vanilla SpMV ... " << std::endl;
  symmetric = false;
  test_spmv(symmetric);
  ...
  if (mpi_rank == 0)
    std::cout << "Running symmetric SpMV ... " << std::endl;
  symmetric = true;
  test_spmv(symmetric);
