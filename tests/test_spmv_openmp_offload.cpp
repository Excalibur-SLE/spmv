...
static bool test_spmv_ptr(bool symmetric)
{
  int mpi_rank;
  ...
  // Compute SpMV
  l2g->update(x_local);
  A->mult(x_local, y_local, spmv::Device::gpu);
...
static bool test_spmv_eigen(bool symmetric)
{
  int mpi_rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank);
  int mpi_size;
  ...
  y_local = A->mult(x_local, spmv::Device::gpu);
...
int main(int argc, char** argv)
{
  ...
  if (mpi_rank == 0)
    std::cout
        << "Running vanilla SpMV on accelerator with pointer interface... "
        << std::endl;
  symmetric = false;
  ret &= test_spmv_ptr(symmetric);

  MPI_Barrier(MPI_COMM_WORLD);

  if (mpi_rank == 0)
    std::cout << "Running vanilla SpMV on accelerator with Eigen interface... "
              << std::endl;
  ret &= test_spmv_eigen(symmetric);

  MPI_Barrier(MPI_COMM_WORLD);

  if (mpi_rank == 0)
    std::cout
        << "Running symmetric SpMV on accelerator with pointer interface... "
        << std::endl;
  symmetric = true;
  ret &= test_spmv_ptr(symmetric);
  ...
  if (mpi_rank == 0)
    std::cout
        << "Running symmetric SpMV on accelerator with Eigen interface... "
        << std::endl;
  ret &= test_spmv_eigen(symmetric);
